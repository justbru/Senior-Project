import { CircularProgress, Button, Link, Card, CardHeader, CardBody, Spacer, Textarea } from '@nextui-org/react';
...
export default function SummaryPage(props) {
    ...
    useEffect(() => {
        ...
        if (props.transcript_original && categorizedText_original === 0) {
            ...
            setOutput_s(tempOutputS.trim())
            setOutput_o(tempOutputO.trim())
            setOutput_a(tempOutputA.trim())
            setOutput_p(tempOutputP.trim())
            setUnclassified(tempUnclassified.trim())
        }
        else
            console.log("waiting for props to load")
    }, [props, output_s]);
            ...
            <AnimatePresence mode="wait" initial={false}>
                {props.isLoading && (
                    <motion.div
                        ...
                        exit={{ opacity: 0 }}
                        key="loading"
                        className="block text-center text-6xl tracking-wide"
                    >
                        <CircularProgress size="lg" color="default" label="Summarizing your meeting..." />
                    </motion.div>
                )
                }
                {!props.isLoading && props.transcript && (
                    ...
                    >
                        <Spacer y={24} />
                        <div className='block text-left text-6xl tracking-wide pt-96'>
                            Review and edit your <br /> consultation summary
                        </div>
                        <Spacer y={12} />
                        ...
                        <Spacer y={12} />
                        <Card>
                            <CardHeader className="bg-brown-50 w-full">
                                Unclassified
                            </CardHeader>
                            <CardBody>
                                {unclassified}
                        ...
                        <Spacer y={16} />
                        <div className='text-xl grid grid-cols-2 gap-4'>
                            <Button className="bg-gradient-to-tr from-brown-100 to-brown-400 text-white shadow-lg w-full" onClick={onCopy} variant="flat" radius="full">
                                Copy SOAP outline
                            </Button>
                            <Button as={Link} className="bg-gradient-to-tr from-brown-100 to-brown-400 text-white shadow-lg" href="trial" variant="flat" radius="full">
                                Summarize another consultation
                            </Button>
                        </div>
                        <Spacer y={24} />
                    </motion.div>
                ...
                {!props.transcript && !props.isLoading && (
                    <motion.div
                        ...
                        exit={{ opacity: 0 }}
                        key="nothing"
                        className='text-xl grid grid-cols-1 gap-4'
                    >
                        No speech detected.
                        <Button as={Link} className="bg-gradient-to-tr from-brown-100 to-brown-400 text-white shadow-lg" href="trial" variant="flat" radius="full">
                            Try again
